...
def fixture(seed=None):
    ...
    html=re.sub(r'assets/cards/([a-z]+)\.svg',lambda m:"data:image/svg+xml;base64,"+base64.b64encode((root/m.group(0)).read_bytes()).decode(),html)
...
with sync_playwright() as p:
    ...
    assert page.evaluate("readyWords().length")==60
    assert page.locator(".category").count()==6
    assert page.evaluate("CATS.map(c=>c.label)")==["동물","과일","야채","탈것","음식","캐릭터"]
    assert page.evaluate("Object.fromEntries(CATS.map(c=>[c.id,readyWords(c.id).length]))")==dict(animals=12,fruit=8,vegetables=10,vehicles=12,food=10,characters=8)
    ok("Six categories and 60 active cards")
    page.evaluate("Promise.all(WORDS.map(w=>new Promise((yes,no)=>{const i=new Image();i.onload=()=>yes();i.onerror=no;i.src=w.art})))")
    ok("All 60 uniform illustrations decode")
